...
export class TelegramBotService {
  ...
  async handleConversationStep(chatId, text, state) {
    if (state.step === "name") {
      ...
      await this.sendMessage(chatId, "שלב 2/3: מה המיקום?");
      return;
    }
    ...
    if (state.step === "location") {
      ...
      if (!locations.includes(location)) {
        await this.sendMessage(chatId, "המיקום לא נמצא ברשימת המיקומים.");
        return;
      }
      ...
      await this.sendMessage(chatId, "שלב 3/3: מה סטטוס ההזנה? תקין או לא תקין");
      return;
    }
